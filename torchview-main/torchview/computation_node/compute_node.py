...
class ModuleNode(Node):
    ...
    def __init__(
        ...
        self.set_node_id()
        self.node_type = 0 # add by kylin at 2023/7/16
    ...
    # add by kylin at 2023/7/16
    def update_node_type(self) -> None:

        input_sum_size = 0
        for input_data_size in self.input_shape:
            size = 1
            for unit in input_data_size:
                size *= unit
            input_sum_size += size

        output_sum_size = 0
        for output_data_size in self.output_shape:
            size = 1
            for unit in output_data_size:
                size *= unit
            output_sum_size += size

        if input_sum_size > output_sum_size:
            self.node_type = 1
